    ...
    public partial class Form1 : Form
    {
        ...
        void pMouseMove(object sender, MouseEventArgs e)
        {
            Point ClickOffset = new Point(e.X - ViewOffset.X, e.Y - ViewOffset.Y),
                  ClickScale = new Point((int)(e.X / ResolutionRatio.Width), (int)(e.Y / ResolutionRatio.Height)),
                  ClickBoth = new Point((int)((e.X - ViewOffset.X) / ResolutionRatio.Width), (int)((e.Y - ViewOffset.Y) / ResolutionRatio.Height));
            if (AnyBallSelected)
            {
                Point[] PP = new Point[4]
                {
                    new Point(ClickBoth.X - 9, ClickBoth.Y - 9),
                    new Point(ClickBoth.X - 9, ClickBoth.Y + 18),
                    new Point(ClickBoth.X + 18, ClickBoth.Y + 18),
                    new Point(ClickBoth.X + 9, ClickBoth.Y - 18)
                };
                for (int b = 0; b < 4; ++b)
                    if (reg.IsVisible(PP[b]) && !new Region(HERO_INITIAL_POSITION).IsVisible(PP[b]))
                        Balls[SelectedBall].Position = ClickBoth;
                    else
                    {
                        SelectedBall = -1;
            ...
            if (SpeedChangerSelected && SpeedChangerRectangle.Contains(ClickScale))
                BallSpeed = (float)Math.Round((ClickScale.X - SpeedChangerRectangle.Left) / 9f, 1);
            if (BallSpeed < BALL_MIN_SPEED)
                BallSpeed = BALL_MIN_SPEED;
            else
                if (BallSpeed > BALL_MAX_SPEED)
                    BallSpeed = BALL_MAX_SPEED;
            if (CountChangerSelected && CountChangerRectangle.Contains(ClickScale))
                BallCount = (int)Math.Round((ClickScale.X - CountChangerRectangle.Left) * 1.1f);
            
            if (BGSpeedChangerSelected && BGSpeedChangerRectangle.Contains(ClickScale))
                BGInterval = (ClickScale.X - BGSpeedChangerRectangle.Left) * 5;
            ...
            MouseAiming = ClickBoth;// ClickOffset;
        }
        ...
        void pMouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (!AnyBallSelected)
            {
                ...
                for (int a = 0; a < Balls.Count; ++a)
                {
                    ...
                    dist = Math.Sqrt(Math.Pow(x - ClickOffset.X / ResolutionRatio.Width, 2) + Math.Pow(y - ClickOffset.Y / ResolutionRatio.Height, 2));
                    if (dist < 15)
                    {
            ...
            if (!SpeedChangerSelected)
                if (SpeedChangerRectangle.Contains(ClickScale) && GameState == ProgramState.Settings)
                    SpeedChangerSelected = true;
            if (!CountChangerSelected)
                if (CountChangerRectangle.Contains(ClickScale) && GameState == ProgramState.Settings)
                    CountChangerSelected = true;
            if (!BGSpeedChangerSelected)
                if (BGSpeedChangerRectangle.Contains(ClickScale) && GameState == ProgramState.Settings)
                    BGSpeedChangerSelected = true;

            if (GameState == ProgramState.Dialog)
            {
                if (OkRectangle.Contains(ClickScale))
                {
                    ...
                    if (ExitRectangle.Contains(ClickScale))
                        Application.Exit();
            }
            pMouseMove(sender, e);
        }
        ...
        {
            ViewOffset.X = -((int)theHero.Position.X - 960);
            ViewOffset.Y = -((int)theHero.Position.Y - 540);
            if (AimingEnabled)
            #region Aiming Information
            {
                ...
                foreach (Ball TB in Balls)
                {
                    ...
                    if (dist2 < 15 && TB.Exist)
                    {
                        ...
                        AimingPosition = new PointF((TB.Position.X + ViewOffset.X - width / 2) / ResolutionRatio.Width, (TB.Position.Y + ViewOffset.Y - height) / ResolutionRatio.Height);
                ...
                foreach (Door TD in Doors)
                {
                    if (TD.Rectangle.Contains(MouseAiming))
                    {
                        ...
                        float width = 9 * iGlassPanelSimple.Width / 10,
                              height = 3 * iGlassPanelSimple.Height / 4;
                        AimingPosition = new PointF((TD.Rectangle.X + ViewOffset.X - width / 2) / ResolutionRatio.Width, (TD.Rectangle.Y + ViewOffset.Y - height) / ResolutionRatio.Height);
                ...
                if (dist2 < 15)
                {
                    ...
                    float width = 4 * iGlassPanelSimple.Width / 5,
                          height = iGlassPanelSimple.Height / 2;
                    AimingPosition = new PointF((theHero.Position.X + ViewOffset.X - width / 2) / ResolutionRatio.Width, (theHero.Position.Y + ViewOffset.Y - 1.4f * height) / ResolutionRatio.Height);
        ...
        void pDraw(object sender, PaintEventArgs e)
        {
            ...
            float x = theHero.Position.X, y = theHero.Position.Y, d = theHero.Direction;switch (theHero.HandsItem)
            {
            ...
            g.DrawString("Lives", Nirmala20, textLB, new Point(31, 1015));
            ...
            switch (GameState)
            {
                ...
                case ProgramState.Settings:
                    ...
                    g.DrawString("Settings", Verdana15, new SolidBrush(Color.FromArgb(60, 60, 60)), 862, 390);
